...
func (d *dir) options() []int {
	...
	for _, dir := range d.dirs {
		result = append(result, dir.options()...)
	}
...
func main() {
	...
	fmt.Println(fileServer.size)
	fmt.Println(70000000 - fileServer.size)
	fmt.Println(30000000 - (70000000 - fileServer.size))

	fmt.Println("options")
	x := fileServer.options()
	sort.Ints(x)

	fmt.Println(x)
	fmt.Println(x[0])
	fmt.Println(fileServer.size - x[0])
